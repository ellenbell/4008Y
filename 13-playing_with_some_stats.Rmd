...
When it comes to investigating relationships between two continuous variables we may want to know if there is a statistical correlation between variables. We can use Pearsons correlation coefficient to do this. Try running the following chunk of code to test for a correlation between height and weight in the athletes data set; 
...
Your output should look comethink like this;
...
Here we can see that R has given us a test statistic `t`, degrees of freedom `DF` and p-value. R even helpfully tells us that it considers the alternative hypothesis true in this case and that there is a correlation between weight and height variables given that the p-value is less than 0.01. We arent going to worry about the other information given in this summary. 

So you have probably heard that correlation doesn't equal causation. But we may wish to see if we can use height to predict weight, to do this we need to try and fit a model to our data. Here we can apply the `lm()` function again. When we have two continuous variables and use the lm() function we are essentially performing a linear regression. The linear regression essentially uses the equation for a straight line;
...
The output here should look familiar, it is very similar to that produced for the general linear models we fitted before. We can interpret the call and residuals sections of the summary as we did before. However the coefficients section is a little different in its interpretation. If we look at the `Estimate` column, the `(Intercept)` is the intercept in the traditional sense, this would be where the regression line meets the Y axis when height is zero. Now of course its impossible for someone to weigh -126kgs even if they have a height of 0cms (which is also impossible). But this is simply the fit of the regression line. Now if you look at the `ht` `Estimate` this is 1.12, this is the slope of the regression line. We can also think of this as for every increment in height, weight increases by 1.12 increments as well. We can test this theory. We can plug these values into the y = mx + c equation to estimate the weight of an athlete with a height of 160cms. Try plugging `1.12 * 160 -126` into the console in R. You should have a scatter plot with a regression line produced in the previous workshop, have a look and see how our regressions prediction of the weight of an athlete with a height of 160cms compares to the plot. 
...
Here our test statistic is W and our p value is fairly self explanatory. But the interpretation of this test is often a little challenging for students. Here the null hypothesis is that our data follow a normal distribution so if our p value is greater then 0.05 we wont reject the null hypothesis and will assume our data are normally distributed. But if our p value is less than 0.05 the distribution of our data is deemed significantly different to a normal distribution so our null hypothesis is rejected and we assume that our data are not normally distributed. So in this instance because the weight vairable follows a normal distribution it would be fine to run a parametric test on it. 
...
So hopefully you can see that log transforming has reduced the p value for red blood cell counts in female athletes it is still greater then 0.05 so the distribution is still not normal. The red blood cell count for male athletes following log transformation is normally distributed. But because some of our data does not follow a normal distribution even after log tranforming it we will need to perform a non parametric test. 
...
Here S is the test statstic and the p-value is, well the p-value. This again shows that there is a correlation between height and red blood cell count. 
